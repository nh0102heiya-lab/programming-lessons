...
変数は、**ラベルが貼られた箱**のようなものです。

```
┌─────────────────┐
│    message      │ ← 箱に貼られたラベル（変数名）
├─────────────────┤
│  "Hello, Rust!" │ ← 箱の中身（値）
└─────────────────┘
```

`let message = "Hello, Rust!";` は、「`message` というラベルの箱に、`"Hello, Rust!"` という値を入れる」という意味です。

箱のラベル（変数名）を使って、中身の値を取り出して使うことができます。
...
`mut` のイメージは、「箱のフタを開けられるようにする」ことです。
